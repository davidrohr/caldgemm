...
int SetupUserData(caldgemm::caldgemm_config &Config)
{
	...
	else
	{
		...
		if (!quietbench) fprintf(stderr, "...alloc A (%lld KB)", (size_t) (height_a * pitch_a * sizeof(double) / 1024));
		AA = dgemm->AllocMemory(height_a * pitch_a, mem_page_lock, mem_huge_table, mem_gpu_access);
		if (!quietbench) fprintf(stderr, "...alloc B (%lld KB)", (size_t) (height_b * pitch_b * sizeof(double)  / 1024));
		BB = dgemm->AllocMemory(height_b * pitch_b, mem_page_lock, mem_huge_table, mem_gpu_access);
		if (!quietbench) fprintf(stderr, "...alloc C (%lld KB)", (size_t) (Config.m * pitch_c * sizeof(double)  / 1024));
